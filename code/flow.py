...
# Reconfigure stdout/stderr to handle UTF-8/fallback on Windows consoles
...
class Graph:
    ...
    def extend_from(self, src_nid: str, result: AgentResult,
        ...
        parent_recovers = self.g.nodes[src_nid].get("metadata", {}).get("recovers")
        for spec in result.successors:
            label = (spec.metadata or {}).get("label")
            node_metadata = dict(spec.metadata or {})
            if parent_recovers:
                node_metadata["recovers"] = parent_recovers
        ...
        for child_skill in src_def.internal_successors:
            nid = self.add_node(child_skill, inputs=[last_nid])
            added.append(nid)
            internal_added.append(nid)
            last_nid = nid

        if internal_added and last_nid != src_nid:
            # Re-route any existing downstream successors of src_nid to wait on the last internal successor instead
            downstream = [c for c in self.g.successors(src_nid) if c not in internal_added]
            for child_nid in downstream:
                self.g.remove_edge(src_nid, child_nid)
                self.g.add_edge(last_nid, child_nid)
                # Replace references to src_nid in child_nid's inputs
                inputs = self.g.nodes[child_nid].get("inputs") or []
                new_inputs = [last_nid if inp == src_nid else inp for inp in inputs]
                self.g.nodes[child_nid]["inputs"] = new_inputs

        # Critic auto-insertion: place a single Critic after a critic-flagged skill,
        # redirecting all of its downstream successors to wait on it.
        if src_def.critic:
            successors = list(self.g.successors(src_nid))
            successors = [c for c in successors if self.g.nodes[c]["skill"] != "critic"]
            if successors:
                critic_nid = self.add_node(
                    "critic", inputs=[src_nid],
                    metadata={"target": src_nid},
                )
                for child_nid in successors:
                    ...
                    # Replace references to src_nid in child_nid's inputs
                    inputs = self.g.nodes[child_nid].get("inputs") or []
                    new_inputs = [critic_nid if inp == src_nid else inp for inp in inputs]
                    self.g.nodes[child_nid]["inputs"] = new_inputs
...
class Executor:
                ...
                if result.success:
                    ...
                    if graph.g.nodes[nid]["skill"] == "formatter":
                        ...
                        if isinstance(fa, str) and fa.strip():
                            ...
                            # Surface the raw output so the issue is visible
                            # rather than silently falling back to json.dumps.
                            print(
                                f"[flow] WARNING: formatter node {nid} succeeded "
                                f"but 'final_answer' key is missing or empty. "
                                f"Raw output keys: {list(result.output.keys())}. "
    ...
    async def _run_one(self, nid: str, graph: Graph, sid: str, query: str,
        ...
        try:
            ...
            print(f"[flow] Node {nid} ({skill_name}) failed with exception:\n{tb}")
