...
def resolve_inputs(node_inputs: list[str], graph_nodes, query: str) -> list[dict]:
    ...
    for inp in node_inputs:
        if inp == "USER_QUERY":
            ...
            target_nid = inp
            upstream = graph_nodes[target_nid].get("result")
            if isinstance(upstream, AgentResult) and upstream.agent_name == "critic":
                # Bypass critic node to retrieve the target writer's actual output
                critic_inputs = graph_nodes[target_nid].get("inputs") or []
                if critic_inputs:
                    target_nid = critic_inputs[0]
                    upstream = graph_nodes.get(target_nid, {}).get("result")

            # If target node is a coder, redirect to its completed sandbox_executor
            if isinstance(upstream, AgentResult) and upstream.agent_name == "coder":
                for nid, d in graph_nodes.items():
                    if (d.get("skill") == "sandbox_executor" 
                            and target_nid in d.get("inputs", []) 
                            and d.get("status") in ("complete", "failed")):
                        target_nid = nid
                        upstream = d.get("result")
                        break

            if isinstance(upstream, AgentResult):
                out.append({"id": target_nid, "kind": "upstream",
...
def parse_skill_json(text: str) -> dict:
    ...
    Handles the two common formats the model emits:
      ```json          ```
      {...}     and    {...}
      ```              ```
    The original strip('`') was broken: it stripped ALL backtick chars from
    both ends of the string, which corrupted JSON starting/ending with
    characters that happen to share position with backtick sequences.
    """
    t = (text or "").strip()
    # Remove opening fence line (``` or ```json or ```JSON etc.)
    if t.startswith("```"):
        first_newline = t.find("\n")
        if first_newline != -1:
            t = t[first_newline + 1:].strip()
        else:
            # Fence with no newline — strip just the backticks
            t = t.lstrip("`").strip()
    ...
    try:
        ...
        # Fallback: extract the outermost JSON object in case there is
        # still surrounding prose (e.g. a model that ignored instructions).
...
async def run_skill(skill: Skill, node_id: str, graph_nodes,
    ...
    if skill.name == "critic" and graph_nodes[node_id]["inputs"]:
        target_nid = graph_nodes[node_id]["inputs"][0]
        if target_nid in graph_nodes:
            target_question = graph_nodes[target_nid].get("metadata", {}).get("question")
            target_inputs = graph_nodes[target_nid].get("inputs") or []
            resolved_target_inputs = resolve_inputs(target_inputs, graph_nodes, query)
            resolved.append({
                "id": f"{target_nid}_inputs",
                "kind": "upstream_inputs",
                "value": resolved_target_inputs
            })
    rendered = render_prompt(skill, query, resolved, failure_report,
                             memory_hits=memory_hits)
    if target_question:
        rendered += f"\n\nTARGET_NODE_SUBTASK: {target_question}"
    started = time.time()
