        ...
        for attempt in range(max_attempts):
            try:
                ...
                if status in (429, 502, 503, 504) and attempt < max_attempts - 1:
                    sleep_time = 1.5 + attempt * 1.5
                    print(f"[client.LLM.chat] transient error {status} from gateway, retrying in {sleep_time}s... (attempt {attempt+1}/{max_attempts})")
                ...
                if attempt < max_attempts - 1:
                    sleep_time = 1.5 + attempt * 1.5
                    print(f"[client.LLM.chat] request error {type(e).__name__}: {e}, retrying in {sleep_time}s... (attempt {attempt+1}/{max_attempts})")
                    time.sleep(sleep_time)
                    continue
                raise
